 ...
 */
export function extractMinutesFromCallMinutes(callMinutes: string | number): number {
 ...
 */
export function getRemainingMinutes(subscription: { secondsLeft: number } | null | undefined): number {
 ...
 */
export function getTotalMinutes(plan: { features: { callMinutes: string | number } } | null | undefined): number {
  if (!plan) return 0;
