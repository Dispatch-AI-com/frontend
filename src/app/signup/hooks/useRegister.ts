...
const handleRegisterError = (error: unknown): string => {
  ...
  if (axiosError.response?.status) {
    return `Server error: ${axiosError.response.status.toString()} - ${axiosError.response.statusText}`;
  }
  ...
  const register = async (
    ...
    try {
      ...
      if (isSuccessfulResponse(response)) {
        handleRegisterSuccess(response.data as RegisterResponse, router);
        return true;
      }
      ...
      const errorMessage =
        ...
        response.data.error ??
        `Registration failed with status ${response.status.toString()}`;
